...
- **Status:** in active development. Version is **0.16.0** in `studio/__init__.py`; the `v0.16.0` tag and GitHub Release are **not cut yet** (newest tag is `v0.15.1`). CI green. **Every version bump must ship a GitHub Release** or the in-app update check never fires. The project was renamed twice (lora-dataset-studio → lora-distillery → dataset-deviser); older references under the previous names are stale.
