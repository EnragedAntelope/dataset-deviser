...
def normalize_suffix(suffix: str) -> str:
    """Lowercase an image extension, collapsing `.jpeg` to `.jpg`.

    Export copies source bytes verbatim, so the extension it writes has to be the
    source's real one — it used to hard-code `.png`, which mislabelled every JPEG
    and WEBP source as PNG. Normalising keeps a folder from ending up half `.JPG`
    and half `.jpeg` for the same format.
    """
...
def package_dataset(
    ...
    for i, (img, caption) in enumerate(items, start=1):
        ...
        shutil.copy2(img, ds_dir / filename)
    ...
    ext_note = "/".join(extensions) or ".png"
    ...
    readme = [
        ...
        f"Images: {len(items)} (each image NN{ext_note} pairs with caption NN.txt)",
        "",
        "Generated by Dataset Deviser.",
...
def package_folder(
    ...
    from studio.config import list_images, read_caption
    ...
    for img in images:
        if img.with_suffix(".txt").exists():
            items.append((img, read_caption(img)))
        else:
            missing.append(img.name)
    if not items:
        raise RuntimeError(
